...
**Caches** (`internal/cache/cache.go`) are package-level singletons backed by `dgraph-io/ristretto/v2`: `ignoreCache`, `resizeCache`, `vidThumbCache`, and `dimensionCache`. They must be initialized (`NewXCache`) before use or the `GetXCache` accessors `log.Fatal`. `Server.initCache` sets them up conditionally based on flags (dimension cache when gallery is on; resize/vidthumb only when those features are on). Cache sizes come from CLI flags in MB (`sizeToNCMB` derives NumCounters heuristically) — except `dimensionCache`, which is a fixed tiny cache (one `image.Point` per entry, cost 1) with no flag.
...
**Infinite scroll and click-to-play videos** (`internal/templates/base.html` gallery JS, `list.html`): with JS, `base.html` drives loading via the Infinite Scroll library (`append: ".grid-item"`, `outlayer: msnry`) and hides the server `.pagination` (`hideNav`); without JS the numbered pagination links are the fallback. Because infinite scroll appends items to the live DOM without removing them, **grid video tiles are lazy** — `list.html` renders each video as a `.video-placeholder` anchor (poster + play badge) that links to the `/post/` view (the no-JS fallback), carrying the raw video URL in `data-video`. A single delegated `click` handler on `.grid` (so it also covers infinite-scroll-appended tiles) replaces the placeholder with a real `<video>` only on click, keeping live media elements bounded to what the user actually plays (issue #38). The `-no-infinite-scroll` flag (`config.NoInfiniteScroll` → `GalleryContext.InfiniteScroll` → `data-infinite` on `.grid`) skips Infinite Scroll init entirely, leaving the server pagination visible even with JS; click-to-play still works. Note this only bounds *video* memory — grid images and DOM nodes still accumulate during infinite scroll (DOM windowing is a deliberate non-goal for now).

**Layout stability while thumbnails generate**: masonry positions tiles by height, so tiles that render at zero height (a video placeholder with no reserved box, or an `<img>` before its thumbnail loads) collapse the grid and make it reflow — which also shoves the footer up the page. Two guards keep the grid's height stable from first render: (1) `.video-placeholder` reserves a `16/9` box up front (CSS in `base.html`); (2) `GalleryHandler` calls `probeImageDimensions` to fill `GalleryItem.Width`/`Height` (header-only `image.DecodeConfig`, cached in `dimensionCache` by path, decoders registered via blank imports in `gallery.go`) so `list.html` emits `width`/`height` on each grid `<img>` and the browser reserves the aspect-ratio box before the image loads. Probing runs only for the current page's image items (bounded by `-split`); undecodable formats (svg) get zero dimensions and are skipped.
